...
formu<-
  formula.text("ajuste8","Death2",a="s(max_pacient_valor_GLU_)",eliminar=c("max_pacient_valor_GLU_","G_DIABETIC"),taulavariables = conductor,dt=TAULA_PLANA_exc)

mod.gam.spline<-
  mgcv::gam(as.formula(formu), data = TAULA_PLANA_exc, family=binomial("logit"))

mod.gam.spline %>% 
  sjPlot::tab_model(digits=3, show.intercept = F,title = "Modelo de Mortalidad con termino spline Glucose", p.style = "numeric_stars")

##############################
#? mirar-ho dema! 06.11.2020!#
##############################
...
mgcViz::getViz(mod.gam.spline) %>% 
  ...
  l_rug() +
  theme_grey() +
  ylab("Odds Ratio")+xlab("Glucose") # Canvi d'escala
  #scale_y_continuous(trans='exp',labels=function(x) round(exp(x),1)) +  # Canvi d'escala

  sjPlot::plot_model(mod.gam.spline,type="pred",terms = c("max_pacient_valor_GLU_","DG2.OB"))
...
# Modelos no parametricos
formu<-
  formula.text("ajuste8","Death_DG.Mechanical_ventialtion=='Si'",a="s(max_pacient_valor_GLU_)",eliminar=c("max_pacient_valor_GLU_","G_DIABETIC"),taulavariables = conductor,dt=TAULA_PLANA_exc)

mod.gam.spline<-
  mgcv::gam(as.formula(formu), data = TAULA_PLANA_exc, family=binomial("logit"))

mod.gam.spline %>% 
  sjPlot::tab_model(digits=3, show.intercept = F,title = "Modelo de Mortalidad /VM con termino spline Glucose", p.style = "numeric_stars")
...
mgcViz::getViz(mod.gam.spline) %>% 
  ...
  l_ciLine(linetype = 3) +
  l_rug() +
  theme_grey() +ylab("Odds Ratio")+xlab("Glucose") # Canvi d'escala
