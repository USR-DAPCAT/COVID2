...
```{r exemple_forestplot, include=T}

# Exemple d'un forest plot d'un model


formula.text("ajuste2","Death_DG.Mechanical_ventialtion=='Si'",taulavariables = conductor,dt=dades)%>% 
  glm(data = dades, family = "binomial") %>% 
  parameters::model_parameters(exponentiate = TRUE) %>% 
  forest.plot.v2(label="Parameter",mean="Coefficient",lower="CI_low",upper="CI_high")
