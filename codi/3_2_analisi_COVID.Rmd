...
&check; Canvi de criteri de DM HB>=6.5 y/o Glucemia >=200 <br/>
&check; Generar tabla plana <br/>
&check; Nuevos modelos <br/>
...
## Factoritzar
dades<-dades %>% mutate_at(extreure.variables("factor",conductor),as.factor)
...
formu<-formula.text("ajuste1","Death2",taulavariables = conductor)
glm(formu, data = dades, family = "binomial") %>% 
  sjPlot::tab_model()

formu<-formula.text("ajuste2","Death2",taulavariables = conductor)
glm(formu, data = dades, family = "binomial") %>% 
  sjPlot::tab_model(auto.label = T)


formu<-formula.text("ajuste3","Death2",taulavariables = conductor)
glm(formu, data = dades, family = "binomial") %>% 
  sjPlot::tab_model()


formu<-formula.text("ajuste4","Death2",taulavariables = conductor)
glm(formu, data = dades, family = "binomial") %>% 
  sjPlot::tab_model()
