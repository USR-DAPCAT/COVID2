...
&check; actualizaciones modelos de mortalidad  <br/>
...
dades_DIAB<-dades%>%filter(DG.DM=="Si")
dades_NO_DIAB<-dades%>%filter(DG.DM=="No")
...
c("ajuste1","ajuste2","ajuste3") %>% 
  ...
  sjPlot::tab_model(digits=3, show.intercept = F,dv.labels = c("M1", "M2","M3"),
                    title = "Modelos de Moratlidad",
                    p.style = "numeric_stars")


c("ajuste4","ajuste5","ajuste6","ajuste7") %>% 
  map(~formula.text(.x,"Death2",taulavariables = conductor,dt=dades))%>% 
  map(~glm(.x,data = dades, family = "binomial")) %>% 
  sjPlot::tab_model(digits=3, show.intercept = F,dv.labels = c("M4", "M5","M6","M7"),
                    title = "Modelos de Moratlidad",
                    p.style = "numeric_stars")


c("ajuste8a") %>% 
  map(~formula.text(.x,"Death2",taulavariables = conductor,dt=dades_DIAB))%>% 
  map(~glm(.x,data = dades, family = "binomial")) %>% 
  sjPlot::tab_model(digits=3, show.intercept = F,dv.labels = c("M8a"),
                    title = "Modelos de Moratlidad Grupo Diabético",
                    p.style = "numeric_stars")

c("ajuste8b") %>% 
  map(~formula.text(.x,"Death2",taulavariables = conductor,dt=dades_NO_DIAB))%>% 
  map(~glm(.x,data = dades, family = "binomial")) %>% 
  sjPlot::tab_model(digits=3, show.intercept = F,dv.labels = c("M8b"),
                    title = "Modelos de Moratlidad Grupo No Diabético",
                    p.style = "numeric_stars")


```

```{r exemple_forestplot1, include=T}

# [[Exemple d'un forest plot d'un model, elimino la fila interceptl]]
# filter 0 fora."Parameters"

c("ajuste1","ajuste2","ajuste3","ajuste4","ajuste5","ajuste6","ajuste7","ajuste8a","ajuste8b") %>% 
  ...
  map(~forest.plot.v2(dadesmodel=.x,
                      ...
                      upper="CI_high",
                      intercept=0,
                      label_X="Diferencias modelos estandarizados (IC del 95%) [Mortalidad]"))
...
c("ajuste8a") %>% 
  ...
  map(~glm(.x,data = dades, family = "binomial")) %>% 
  sjPlot::tab_model(digits=3, show.intercept = F,dv.labels = c("M8a"),
                    title = "Muerte y/o Ventilación mecánica Grupo de diabeticos",
                    p.style = "numeric_stars")
...
c("ajuste8b") %>% 
  ...
  map(~glm(.x,data = dades, family = "binomial")) %>% 
  sjPlot::tab_model(digits=3, show.intercept = F,dv.labels = c("M8b"),
                    title = "Muerte y/o Ventilación mecánica Grupo de no diabeticos",
                    p.style = "numeric_stars")


```

```{r exemple_forestplot2, include=T}

# Exemple d'un forest plot d'un model


c("ajuste1","ajuste2","ajuste3","ajuste4","ajuste5","ajuste6","ajuste7","ajuste8a","ajuste8b") %>% 
  map(~formula.text(x=.x,"Death_DG.Mechanical_ventialtion=='Si'",taulavariables = conductor,dt=dades)) %>% 
  map(~glm(formula=.x,data = dades, family = "binomial")) %>% 
  map(~parameters::model_parameters(.x,exponentiate = TRUE))%>% 
  map(~forest.plot.v2(dadesmodel=.x,
                      label="Parameter",
                      ...
                      upper="CI_high",
                      intercept=0,
                      label_X="Diferencias modelos estandarizados (IC del 95%) [Muerte y/o Ventilación mecánica] "))

# preguntar Jordi sobre Labels!  
...
```{r exemple_forestplot3, include=T}

# Exemple d'un forest plot d'un model



c("ajuste1","ajuste2","ajuste3","ajuste4","ajuste5","ajuste6") %>% 
  map(~formula.text(x=.x,"Death_DG.Mechanical_ventialtion_Ad_UCI=='Si'",taulavariables = conductor,dt=dades)) %>% 
  map(~glm(formula=.x,data = dades, family = "binomial")) %>% 
  map(~parameters::model_parameters(.x,exponentiate = TRUE))%>% 
  map(~forest.plot.v2(dadesmodel=.x,
                      label="Parameter",
                      ...
                      upper="CI_high",
                      intercept=0,
                      label_X="Diferencias modelos estandarizados (IC del 95%) [Muerte y/o Ventilación mecánica y/o UCI]"))
